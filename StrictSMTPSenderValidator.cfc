	...
	public struct function isSendersIPAllowedForEmailAddress(
		...
		required string heloSMTPString="" ) {
			...
			*/
            ...
            //if domain part of email address is only an IP4 instead of a domain name
            if( REMatch("^((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)(\.(?!$)|$)){4}$", listLast( local.emailAddress, "@" ) ).len() ){
                    
                appendDebugLogLine( "EmailAddress '#encodeforHTML(local.emailAddress)#' is syntatically valid, but email address has no valid domain name (is an IP4 address)" );
                
                // break here and send component data
                this.resultSMTPVerifier[ "reason" ]="Email-address after the @ consists of just an IP address";
                this.resultSMTPVerifier[ "result" ]= false;
                return this.resultSMTPVerifier;
	...
	*/
	private boolean function hasSendersIPPTR( 
		required string ipAddress ){
			...
			if ( local.PTRDomainDNSEntry != '' ){
				appendDebugLogLine( "senders IP has an PTR Entry '#local.PTRDomainDNSEntry#'" );
				return true;
			} else {
				appendDebugLogLine( "senders IP has NO PTR Entry at all" );
				return false;	
			};
	}
					...
					for ( local.spfitem in local.SpfListArray ) {
                        ...
                        if ( left( local.spfitem, len( "ptr:" ) ) == "ptr:" ) {
                            ...
                            if( isSendersIPAllowedByPTR( local.ipAddress, local.tmpDomainName) ){
                                appendDebugLogLine(  "PTR check: sender #local.ipAddress# is explicity allowed for '#local.tmpDomainName#': true");
                                return true;
                            }else{
                                appendDebugLogLine(  "PTR check: sender #local.ipAddress# is NOT explicity allowed for '#local.tmpDomainName#': false");
